...
#pragma omp parallel
    ...
    nthreads = 1;
        ...
        if (tid == 0)
        {
            ...
            TBC = (DOUBLE_T *)calloc(N, sizeof(DOUBLE_T));
#ifdef _OPENMP
...
#pragma omp barrier
       ...
       vertex */
        ...
        if (tid == 0)
        {
            ...
            // in_degree = (LONG_T *)calloc(n , sizeof(LONG_T));
...
#pragma omp for
        ...
        {
            // MPI_Barrier(MPI_COMM_WORLD);
            MPI_Allreduce(TBC, BC, N, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    if (rank == 0)
    {
        if (SCALE <= 5)
        {
            ...
            fprintf(stderr, "Betweenness centrality: \n");
            for (i = 0; i < N; i++)
            {

                fprintf(stderr, "\tBetweenness centrality %d: %f\n", i, BC[i]);
            }
        }
    }
    return elapsed_time;
